...
const allCourses = [
  ...
  function getThreecourse() {
      return `
        ...
        document.getElementById('courses').innerHTML += ${getCourseAsHtmlString(allCourses[2])}`;
  }
 */

const getThreecourse = () => {
    eleCourse = document.getElementById('courses');
    eleCourse.innerHTML = allCourses.slice(0,3).map(getCourseAsHtmlString).join('\n');
}

document.getElementById('btn').addEventListener('click', getThreecourse)
